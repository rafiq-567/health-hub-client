...
const AdvertiseRequest = ({ sellerEmail }) => {
    ...
    const { data: ads = [], refetch, isLoading } = useQuery({
        queryKey: ['advertised-medicines', sellerEmail],
        queryFn: async () => {
            // Use the new API route with the seller's email as a query parameter
            const res = await axios.get(`https://server-two-rosy-34.vercel.app/advertised-medicines/seller?email=${sellerEmail}`);
            return res.data;
        }
    });
